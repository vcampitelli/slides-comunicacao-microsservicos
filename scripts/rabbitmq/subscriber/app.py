...
if __name__ == '__main__':
    ...
    def callback(ch, method, properties, body):
        try:
            row = json.loads(body)
            print("%s%s%s Email recebido da fila %s" % (Style.NORMAL, Back.GREEN, Fore.WHITE, Style.RESET_ALL))
            print("%s%s Para %s    %s" % (Style.NORMAL, Back.BLACK, Back.RESET, row['to']))
            print("%s%s Assunto %s %s" % (Style.NORMAL, Back.BLACK, Back.RESET, row['subject']))
            print("%s%s Corpo %s   %s" % (Style.NORMAL, Back.BLACK, Back.RESET, row['body']))
            print("")
    ...
    print('Waiting for messages in exchange "%s" for queue "%s"' % (exchange, queue))
